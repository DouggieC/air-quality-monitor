...
class AirQualityClient:
    ...
    def get_all_cities(self, country, state):
        ...
        params = {"country": country, "state": state, "key": self.api_key}
        self.logger.debug(f"Parameters:\t{params}")
...
class WeatherClient:
    ...
    def get_coordinates(self, city: City, limit=None) -> dict:
        ...
        params = {"q": city.city, "appid": self.api_key, "limit": limit}
        self.logger.debug(f"Parameters:\t{params}")
        ...
        if data.get("code") is not None or data.get("cod") is not None:
            raise APIError(
                f"OWM API failed for city {city.city}: error code {data.get('code')}: {data.get('message')}"
            )
    ...
    def get_location(self, lat, lon, limit=None):
        ...
        params = {"lat": lat, "lon": lon, "appid": self.api_key, "limit": limit}
        self.logger.debug(f"Parameters:\t{params}")
        ...
        if data.get("code") is not None or data.get("cod") is not None:
            raise APIError(
                f"OWM API failed for coords [{lat},{lon}]: error code {data.get('code')}: {data.get('message')}"
            )
    ...
    def get_current_weather(self, *, lat=None, lon=None, city: City | None = None):
        ...
        if city:
            self.logger.debug("City name supplied. Getting coordinates...")
